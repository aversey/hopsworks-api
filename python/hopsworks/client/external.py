...
from hopsworks.client.exceptions import FeatureStoreException
...
class Client(base.Client):
    ...
    def __init__(
        ...
        if api_key_value is not None:
            ...
            _logger.debug("Using provided API key file")
            file = None
    ...
    def _get_ca_chain_path(self, project_name=None) -> str:
        if project_name is not None:
            self._cert_folder = os.path.join(self._cert_folder_base, project_name)
    ...
    def _get_secret(self, secrets_store, secret_key=None, api_key_file=None):
        ...
        :raises hopsworks.client.exceptions.ExternalClientError: `api_key_file` needs to be set for local mode
        :raises hopsworks.client.exceptions.UnknownSecretStorageError: Provided secrets storage not supported
        :return: secret
        :rtype: str
        """
